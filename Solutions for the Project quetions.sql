...
SELECT (tech_products_sold / total_distinct_product_ids) * 100 AS percentage_of_total_products
FROM TechProductsSold, TotalProducts;
-- 10,288
...
from 
    (select count(*) as tech_products_sold
        ...
        where product_category_name_english 
            in ('audio','auto','air_conditioning','consoles_games','home_appliances','home_appliances_2','electronics','pc_gamer','computers','signaling_and_security','tablets_printing_image','telephony','fixed_telephony','computers_accessories','small_appliances')
    ) as tech,
    (
...
-- Query to classify and count orders based on delivery status
SELECT
    CASE
...
-- Selects the delay range and the count of orders from the subquery result
SELECT x.delay_range, x.orders_count
FROM
(
    -- Subquery to calculate delay ranges and associated statistics
    SELECT
        -- Case statement to classify orders into delay ranges based on the difference between the estimated and actual delivery dates
        CASE 
            ...
            WHEN DATEDIFF(order_delivered_customer_date, order_estimated_delivery_date) >= 7 AND DATEDIFF(order_estimated_delivery_date, order_delivered_customer_date) < 15 THEN "7-14 day delay"
            WHEN DATEDIFF(order_delivered_customer_date, order_estimated_delivery_date) > 3 AND DATEDIFF(order_estimated_delivery_date, order_delivered_customer_date) < 7 THEN "4-7 day delay"
            WHEN DATEDIFF(order_delivered_customer_date, order_estimated_delivery_date) >= 1 AND DATEDIFF(order_estimated_delivery_date, order_delivered_customer_date) <= 3 THEN "1-3 day delay"
            WHEN DATEDIFF(order_delivered_customer_date, order_estimated_delivery_date) > 0 AND DATEDIFF(order_estimated_delivery_date, order_delivered_customer_date) < 1 THEN "less than 1 day delay"
            WHEN DATEDIFF(order_delivered_customer_date, order_estimated_delivery_date) <= 0 THEN 'On time' 
        END AS "delay_range",  -- Assign the resulting delay range to the column delay_range
        
        -- Additional statistics for each delay range
        AVG(product_weight_g) AS weight_avg,  -- Calculate average product weight for each delay range
        MAX(product_weight_g) AS max_weight,  -- Calculate maximum product weight for each delay range
        MIN(product_weight_g) AS min_weight,  -- Calculate minimum product weight for each delay range
        SUM(product_weight_g) AS sum_weight,  -- Calculate total product weight for each delay range
        COUNT(DISTINCT a.order_id) AS orders_count  -- Count the number of distinct orders for each delay range

    -- Specify the tables to join and their aliases
    FROM orders a
    LEFT JOIN order_items b
        USING (order_id)  -- Join with order_items on order_id
    LEFT JOIN products c
        USING (product_id)  -- Join with products on product_id

    -- Filters to ensure only valid delivered orders are considered
    WHERE order_estimated_delivery_date IS NOT NULL
    AND order_delivered_customer_date IS NOT NULL
    AND order_status = 'delivered'

    -- Group results by the calculated delay ranges
    GROUP BY delay_range
) x;  -- Alias the subquery as x


-- >1 Month Delay = 2,709
-- >14 Days = 38,741
-- >1-2 week delay = 34,215
-- 4-7 day delay = 8,076
-- 